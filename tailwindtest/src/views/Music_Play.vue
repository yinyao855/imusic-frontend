<template>
  <div class="outcontainer">
      <div class="card">
        <label for="uploadx">
          ...
          <div class="card__wrapper">
            ...
            <div class="card__timeline">
              <progress :value="currentduration" :max="maxlength"></progress>
            </div>
        ...
        <audio ref="audioPlayer" :src="audioSrc" @timeupdate="getCurrentTime" @loadedmetadata="getDuration"></audio>
      </div>
  </div>
  <ul>
    <span v-for="(item, index) in lyricsshow" :key="index" :class="{ 'highlighted': item.special }">
      {{ item.text }}
      <br>
    </span>
  </ul>
...
const getCurrentTime = () => {
  ...
  currentTimex.value = minute * 60 + second;
  displayLyrics(lyrics, currentTimex.value);
};
